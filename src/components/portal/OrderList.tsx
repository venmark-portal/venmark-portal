...
function getItemAvailStatus(
  ...
  // Bestillings-frist-tidspunkt: Åbn Til (daglig frist) ELLER — for auktionsvarer der typisk kun
  // har Auktions-frist — auktions-fristen. Vises altid som info på varen ("Bestil … inden HH:MM").
...
export default function OrderList({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sticky top-[52px] z-20 md:top-[65px]">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl bg-white px-4 py-2.5 shadow-sm ring-1 ring-gray-200 transition hover:ring-blue-300 disabled:cursor-default disabled:hover:ring-gray-200"
        >
          <span className="relative shrink-0">
            <ShoppingCart size={20} className={totalLines > 0 ? 'text-blue-600' : 'text-gray-300'} />
            {totalLines > 0 && (
              <span className="absolute -right-2 -top-2 min-w-[18px] rounded-full bg-blue-600 px-1 text-center text-[10px] font-bold leading-[18px] text-white">
                {totalLines}
              </span>
            )}
          </span>
          <span className="text-sm font-bold text-gray-900">Kurv</span>
          <span className="text-xs text-gray-400">
            {totalLines === 0 ? 'Tom' : `${totalLines} ${totalLines === 1 ? 'linje' : 'linjer'}`}
          </span>
          <span className="ml-auto flex items-center gap-3">
            {totalAmount > 0 && <span className="text-sm font-semibold text-gray-900">{fmt.format(totalAmount)}</span>}
            {totalLines > 0 && <span className="text-xs font-semibold text-blue-600">Gennemse →</span>}
          </span>
        </button>
      </div>
      ...
      {showReview && (
        <div ref={reviewRef} className="rounded-xl bg-white ring-2 ring-blue-500 overflow-hidden">
          ...
          <div className="bg-blue-600 px-4 py-3 flex items-center justify-between">
            <div>
              <h2 className="flex items-center gap-2 text-base font-bold text-white"><ShoppingCart size={18} />Kurv</h2>
